...
export function buildUrlElement(route: Route): string {
  ...
  addUrlMetadata(parts, route);
  addExtensionElements(parts, route);
...
export function buildVideoElement(video: Video): string {
  ...
  addVideoLocations(parts, video);
  addVideoTimingAndStats(parts, video);
  addVideoRestrictions(parts, video);
  addVideoMetadata(parts, video);
...
/**
 * Add extension elements (images, videos, news, alternates) to parts array.
 *
 * @param parts - Array to append elements to
 * @param route - Route containing extension data
 *
 * @since 0.2.2
 */
function addExtensionElements(parts: string[], route: Route): void {
  if (route.images?.length) {
    for (const image of route.images) {
      parts.push(buildImageElement(image));
    }
  }
  if (route.videos?.length) {
...
/**
 * Add optional URL metadata fields to parts array.
 *
 * @param parts - Array to append elements to
 * @param route - Route containing metadata
 *
 * @since 0.2.2
 */
function addUrlMetadata(parts: string[], route: Route): void {
  if (route.lastmod) {
 ...
 * @since 0.2.2
 */
function addVideoLocations(parts: string[], video: Video): void {
...
/**
 * Add video metadata fields (uploader, live, tags).
 *
 * @param parts - Array to append elements to
 * @param video - Video data
 *
 * @since 0.2.2
 */
function addVideoMetadata(parts: string[], video: Video): void {
  if (video.uploader) {
...
/**
 * Add video restriction and access fields.
 *
 * @param parts - Array to append elements to
 * @param video - Video data
 *
 * @since 0.2.2
 */
function addVideoRestrictions(parts: string[], video: Video): void {
  ...
  if (video.restriction) {
    const { countries, relationship } = video.restriction;
    parts.push(
      `    <video:restriction relationship="${relationship}">${countries.join(" ")}</video:restriction>`,
    );
  }
  if (video.platform) {
    const { platforms, relationship } = video.platform;
    parts.push(
      `    <video:platform relationship="${relationship}">${platforms.join(" ")}</video:platform>`,
    );
  }
...
/**
 * Add video timing and stats fields.
 *
 * @param parts - Array to append elements to
 * @param video - Video data
 *
 * @since 0.2.2
 */
function addVideoTimingAndStats(parts: string[], video: Video): void {
